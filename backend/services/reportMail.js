...
function buildReport(lang, d) {
  ...
  const kcell = (n, label, dl) => `<td width="25%" valign="top" style="background:#F6F8FB;border-top:3px solid ${accent};padding:14px 6px 12px;text-align:center"><div style="${F}font-size:30px;font-weight:800;color:#0F172A;line-height:1">${esc(n)}</div><div style="${F}font-size:11.5px;color:#64748B;margin:7px 0 5px;line-height:1.3">${esc(label)}</div><div style="${F}font-size:11.5px;font-weight:700">${dl}</div></td>`;
  const k = d.kpi || {}, p = d.prev || {};
  const kpis = `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border-collapse:separate;border-spacing:6px 0;margin:0 -6px"><tr>${kcell(k.touches || 0, X.kTouch, delta(k.touches, p.touches))}${kcell(k.contacted || 0, X.kReach, delta(k.contacted, p.contacted))}${kcell(k.replies || 0, X.kRep, delta(k.replies, p.replies))}${kcell(k.agendadas || 0, X.kMeet, delta(k.agendadas, p.agendadas))}</tr></table>
