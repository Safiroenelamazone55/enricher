...
function _sanReaccion(v) { return STEP_REACCIONES.includes(v) ? v : ''; }
...
const _lmDashHandler = async (req, res) => {
  ...
  try {
    ...
    const prevFrom = new Date(new Date(from).getTime() - days * 864e5).toISOString().slice(0, 10);
    ...
    const [k1, k0, m1, m0, daily, countries, byCh, heat, seqs, clients, funnel, repCh, repDays, recent, heatAuto, deals, dispo] = await Promise.all([
      ...
      pool.query(`SELECT COUNT(DISTINCT k.id) FILTER (WHERE EXISTS(SELECT 1 FROM lm_contact_sequences x WHERE x.contact_id=k.id))::int AS enrolados,
                         ...
                         COUNT(DISTINCT k.id) FILTER (WHERE EXISTS(SELECT 1 FROM activities z WHERE z.contact_id=k.id AND z.tipo='respuesta') OR k.disposition IN ('respondio','reunion','mas_adelante','derivado','no_es_persona','no_interesado','no_contactar') OR (COALESCE(k.disposition,'')='' AND k.estado IN ('respondio','propuesta','negociacion','ganado')))::int AS respondieron,
...
app.post('/api/lm/import', requireAuth, upload.single('file'), async (req, res) => {
  ...
  async function _co(f) {
    ...
    const ins = await pool.query(`
      INSERT INTO lm_companies (user_id,nombre,dominio,website,industria,tamano,ingresos,telefono,linkedin,ciudad,region,pais,fundada,direccion,codigo_postal,descripcion,tecnologias,funding,target_tier,segmento,analisis,outbound_client_id,notas,import_batch)
    ...
    `, [uid, nombre || dominio, dominio, _lmCleanUrlish(f.website), _lmS(f.industria), _lmS(f.tamano), _lmS(f.ingresos),
        ...
        _lmS(f.direccion), _lmS(f.codigo_postal), _lmS(f.descripcion), _lmS(f.tecnologias), _lmS(f.funding), _lmS(f.target_tier), _lmS(f.segmento), _lmS(f.analisis), obcId, _lmS(f.notas), req.file.originalname || '']);
  ...
  for (const row of dataRows) {
    ...
    try {
      if (target === 'companies') {
        ...
        await pool.query(`
          INSERT INTO lm_contacts (user_id,company_id,nombre,apellido,email,email_personal,telefono,movil,cargo,seniority,departamento,linkedin,empresa_nombre,ciudad,region,pais,estado,fuente,contact_priority,buyer_role,analisis,outbound_client_id,notas,raw,import_batch)
        ...
        `, [uid, companyId, nombre, apellido, email, _lmS(f.email_personal), _lmS(f.telefono), _lmS(f.movil), _lmS(f.cargo), _lmS(f.seniority), _lmS(f.departamento),
            _lmS(f.linkedin), _lmS(f.co_nombre), _lmS(f.ciudad), _lmS(f.region), _lmS(f.pais), _lmS(f.estado) || 'nuevo', _lmS(f.fuente) || 'import', _lmS(f.contact_priority), _lmS(f.buyer_role), _lmS(f.analisis), obcId, _lmS(f.notas), JSON.stringify(raw), req.file.originalname || '']);
...
// Base global: TODO lo que trae un archivo importado, no solo la empresa —
// pedido explícito 2026-09-06: "que tenga todos los filtros y opciones que
// tiene un archivo al ser importado... búsqueda nombre por coincidencia,
// apellido, empresa, etc." Une contactos (CRM + cualquier borrador) con su
// empresa, más las empresas que no tienen ningún contacto todavía (para no
// perderlas), en una sola lista buscable por nombre/apellido/cargo/email del
// contacto o nombre/dominio de la empresa.
// Una sola empresa de un borrador, con todo lo que la modal de "Validación
// manual" necesita (mismo shape que /api/cantera/mesa/companies: c.*) —
// pedido explícito 2026-09-25: editar la Validación profunda (Tier/Prioridad/
// Confianza/Nota) directo desde Base Global, sin pasar por Mesa de trabajo.
app.get('/api/cantera/companies/:id', requireAuth, async (req, res) => {
  try {
    const { rows } = await pool.query(
      `SELECT c.*, b.nombre AS batch_nombre FROM cantera_companies c JOIN cantera_batches b ON b.id=c.batch_id WHERE c.id=$1 AND c.user_id=$2`,
      [req.params.id, req.workspaceOwnerId]);
    if (!rows.length) return res.status(404).json({ error: 'Empresa no encontrada' });
    res.json(rows[0]);
...
app.get('/api/cantera/global', requireAuth, async (req, res) => {
  ...
  // Secuencia pasa de texto libre a lista exacta (pedido explícito 2026-09-25:
  // "deberían estar las secuencias activas pausadas todas y yo poder incluir
  // o excluir, para contactos y para empresas") — igual que el resto de
  // filtros tipo lista, con Incluir/Excluir por valor.
  const secuencias = String(req.query.secuencia || '').split(',').filter(Boolean);
  const secuenciasExcl = String(req.query.secuenciaExcl || '').split(',').filter(Boolean);
  ...
  const seniorities = String(req.query.seniority || '').split(',').filter(Boolean);
  const senioritiesExcl = String(req.query.seniorityExcl || '').split(',').filter(Boolean);
  const departamentos = String(req.query.departamento || '').split(',').filter(Boolean);
  const departamentosExcl = String(req.query.departamentoExcl || '').split(',').filter(Boolean);
  const ciudades = String(req.query.ciudad || '').split(',').filter(Boolean);
  const ciudadesExcl = String(req.query.ciudadExcl || '').split(',').filter(Boolean);
  ...
  try {
    ...
    const { rows: all } = tipo === 'empresa' ? await pool.query(`
      SELECT '' AS nombre, '' AS apellido, '' AS cargo, '' AS email,
             ...
             '' AS seniority, '' AS departamento, lco.ciudad AS ciudad, lco.target_tier AS tier, '' AS paso1_estado, '' AS paso2_estado, NULL::int AS company_id, NULL::int AS batch_id
    ...
    `, [uid, q]) : await pool.query(`
      SELECT lc.nombre, lc.apellido, lc.cargo, lc.email,
             ...
             lc.seniority AS seniority, lc.departamento AS departamento, lc.ciudad AS ciudad, co.target_tier AS tier, '' AS paso1_estado, '' AS paso2_estado, NULL::int AS company_id, NULL::int AS batch_id
      ...
      SELECT '', '', '', '', lco.nombre AS empresa, lco.dominio, lco.pais, lco.industria, lco.tamano, 'crm' AS origen,
             ...
             '' AS estado, 'empresa' AS tipo, '' AS secuencias, '' AS seniority, '' AS departamento, lco.ciudad AS ciudad, lco.target_tier AS tier, '' AS paso1_estado, '' AS paso2_estado, NULL::int AS company_id, NULL::int AS batch_id
        FROM lm_companies lco
    ...
    const filtered = all.filter(r => {
      ...
      if (departamentosExcl.length && departamentosExcl.includes(r.departamento || '')) return false;
      if (ciudades.length && !ciudades.includes(r.ciudad || '')) return false;
      if (ciudadesExcl.length && ciudadesExcl.includes(r.ciudad || '')) return false;
      return true;
    });
...
// `company_ids` (opcional): si viene, FUERZA la (re)investigación de justo
// esas empresas sin importar su paso2_estado actual — pedido explícito
// 2026-09-06: "marco 30, 5 ya se investigaron (manual o con IA)... quiero un
// aviso preguntando si aun así deseo continuar, y que se procesen las 30".
// Sin `company_ids` se mantiene el comportamiento de siempre: todo el
// borrador, solo lo que sigue 'pendiente'.
app.post('/api/cantera/batches/:id/run-validacion', requireAuth, async (req, res) => {
  ...
  runBatchValidation(pool, uid, batchId, { onProgress: p => { job.done = p.done; }, companyIds })
...
// Fusiona dos chats que en realidad son la MISMA persona — caso real 2026-09-02
// (Martin Hilson): WhatsApp empezó a reportar su conversación por un jid "@lid"
// nuevo a mitad de camino, así que quedó partida en dos: la vieja (con historial,
// asignada) dejó de recibir mensajes, y los nuevos entraban en un chat "sin
// asignar" aparte — un mensaje real de cliente quedaba invisible en Mías.
// Se queda con el jid que tiene el mensaje MÁS RECIENTE (ese es el canal vivo
// para seguir enviando) y le copia encima los metadatos (asignado/estado/
// prioridad/etiquetas/notas) del otro si el primero no los tenía. Los mensajes
// del jid viejo se REASIGNAN al jid final — no es una vista fusionada en cada
// consulta, después de esto solo existe un chat_jid real.
app.post('/api/wa/connections/:id/chats/:jid/fusionar', requireAuth, async (req, res) => {
  const otroJid = String(req.body?.otroJid || '').trim();
  if (!otroJid || otroJid === req.params.jid) return res.status(400).json({ error: 'Falta el otro chat a fusionar' });
  try {
    const conn = await _cargarConexionAutorizada(req, res, req.params.id);
    if (!conn) return;
    const jidA = req.params.jid, jidB = otroJid;
    const { rows: [ultA] } = await pool.query(`SELECT MAX(ts) AS ts FROM wa_messages WHERE connection_id=$1 AND chat_jid=$2`, [conn.id, jidA]);
    const { rows: [ultB] } = await pool.query(`SELECT MAX(ts) AS ts FROM wa_messages WHERE connection_id=$1 AND chat_jid=$2`, [conn.id, jidB]);
